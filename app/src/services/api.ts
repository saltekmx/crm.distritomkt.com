...
export const studioApi = {
  ...
  generateImage: (data: {
    ...
    seed?: number
  }) => api.post<StudioGeneration>('/studio/image/generate', data),

  // Project generations
  projectGenerations: (projectId: number) =>
    api.get<StudioGeneration[]>(`/studio/project/${projectId}/generations`),

  // Recent generations (all projects)
  recent: () =>
    api.get<StudioGeneration[]>('/studio/recent'),

  // Generation actions
  exportGeneration: (generationId: number) =>
    api.post<{ mensaje: string; media_id: number }>(`/studio/generations/${generationId}/export`),

  deleteGeneration: (generationId: number) =>
    api.delete<StudioGeneration>(`/studio/generations/${generationId}`),

  toggleFavorite: (generationId: number) =>
    api.patch<{ is_favorite: boolean }>(`/studio/generations/${generationId}/favorite`),

  updateTags: (generationId: number, tags: string[]) =>
    api.put<{ tags: string[] }>(`/studio/generations/${generationId}/tags`, { tags }),

  // AI image operations
  removeBackground: (generationId: number) =>
    api.post<StudioGeneration>(`/studio/generations/${generationId}/remove-bg`),

  upscaleImage: (generationId: number, scale: number) =>
    api.post<StudioGeneration>(`/studio/generations/${generationId}/upscale`, { scale }),

  autoEnhance: (generationId: number) =>
    api.post<StudioGeneration>(`/studio/generations/${generationId}/enhance`),

  // Bulk download (returns ZIP blob)
  bulkDownload: (generationIds: number[]) =>
    api.post('/studio/generations/bulk-download', { generation_ids: generationIds }, {
      responseType: 'blob',
    }),

  // Push to pipeline
  pushToPipeline: (generationId: number, pipelineId: number) =>
    api.post<{ mensaje: string; asset_id: number }>(
      `/studio/generations/${generationId}/push-to-pipeline`,
      { pipeline_id: pipelineId },
    ),

  // Resize export
  resizeExport: (generationId: number, data: { width: number; height: number; output_format?: string }) =>
    api.post(`/studio/generations/${generationId}/resize`, data, {
      responseType: 'blob',
    }),

  // Describe image
  describeImage: (imageUrl: string, imageKey?: string | null) =>
    api.post<{ description: string; suggested_prompt: string }>('/studio/describe-image', {
      image_url: imageUrl,
  ...
  // Models
  getModels: () =>
    api.get<{ models: Array<{ id: string; name: string; api_type: string; max_batch: number; supports_editing: boolean; aspect_ratios: string[]; price_hint: string }> }>('/studio/models'),

  // Prompt enhancement
  enhancePrompt: (data: { prompt: string; style_preset?: string | null }) =>
    api.post<{ enhanced_prompt: string }>('/studio/prompt/enhance', data),

  // Templates
  getTemplates: () =>
    api.get<StudioTemplate[]>('/studio/templates'),
  ...
  createTemplate: (data: {
    ...
    aspect_ratio?: string
    is_shared?: boolean
  }) => api.post<StudioTemplate>('/studio/templates', data),

  deleteTemplate: (id: number) =>
    api.delete<StudioTemplate>(`/studio/templates/${id}`),

  // Studio AI Chat (SSE stream URL — use with fetch, not axios)
  chatStreamUrl: `${API_BASE}/studio/chat`,

  // Conversations
  listConversations: (projectId: number) =>
    api.get<Array<{ id: number; titulo: string; creado_en: string; actualizado_en: string }>>(
      `/studio/conversations/${projectId}`,
    ),

  getConversation: (projectId: number, conversationId: number) =>
    api.get<{ id: number; titulo: string; mensajes: Array<{ id: number; rol: string; contenido: string; accion?: unknown; creado_en: string }> }>(
      `/studio/conversations/${projectId}/${conversationId}`,
    ),

  updateConversation: (projectId: number, conversationId: number, data: { titulo?: string }) =>
    api.patch(`/studio/conversations/${projectId}/${conversationId}`, data),
...
export const pipelineApi = {
  ...
  generate: (pipelineId: number, data: { scene_ids: number[] | null; quality?: string }) =>
    api.post(`/pipeline/${pipelineId}/generate`, data),

  revise: (pipelineId: number, data: { scene_id: number; feedback: string; regenerate?: boolean }) =>
    api.post(`/pipeline/${pipelineId}/revise`, data),
  ...
  deleteScene: (sceneId: number) =>
    api.delete(`/pipeline/scenes/${sceneId}`),

  duplicateScene: (sceneId: number) =>
    api.post<PipelineScene>(`/pipeline/scenes/${sceneId}/duplicate`),

  reorderScenes: (pipelineId: number, sceneIds: number[]) =>
  ...
  // Assets
  uploadAsset: (pipelineId: number, file: File) => {
    const formData = new FormData()
    formData.append('file', file)
    return api.post<PipelineAsset>(`/pipeline/assets/upload?pipeline_id=${pipelineId}`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
  },
